...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.count = 0
        self.weight1 = torch.ones(1, device = self.device) * 0.5
        self.weight2 = torch.ones(1, device = self.device) * 0.5
        self.weight_eta = 5e-1
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            weights = self.weight2 + self.weight1
            loss1 = self.weight1 / weights
            loss2 = self.weight2 / weights
            if math.isnan(loss1) or math.isnan(loss2):
                raise ValueError
            ...
            weights = self.weight1 * torch.exp(-self.weight_eta * loss1) + self.weight2 * torch.exp(-self.weight_eta * loss2) + 1e-10
            self.weight1 = self.weight1 * torch.exp(-self.weight_eta * loss1) / weights + 1e-10
            self.weight2 = self.weight2 * torch.exp(-self.weight_eta * loss2) / weights + 1e-10
