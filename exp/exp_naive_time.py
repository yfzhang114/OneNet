...
class net(nn.Module):
    def __init__(self, args, device, revin=False):
        super().__init__()
        self.device = device
        
        self.encoder = TSEncoderTime(input_dims=args.seq_len,
        ...
        self.revin = revin
        if revin: self.revin_layer = RevIN(args.enc_in, affine=False, subtract_last=True, last_n=5)
...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        assert self.online in ['none', 'full', 'regressor', 'encoder']
