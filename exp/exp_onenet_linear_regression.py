...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.count = 0
        if self.individual:
            self.weight = torch.ones((args.enc_in, 2), device = self.device) * 0.5
        else:
            self.weight = torch.ones(2, device = self.device) * 0.5
    ...
    def train(self, setting):
        ...
        self.opt_w = optim.Adam([self.weight], lr=5e-3)
    ...
    def linear_regression(self, x1, x2, y):
        x1 = x1.detach().cpu().squeeze().numpy()
        x2 = x2.detach().cpu().squeeze().numpy()
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            ...
            if self.individual:
                loss1 = self.weight[:,0].view(1, 1, -1)
                loss1 = loss1.repeat(b, t, 1)
                loss1 = rearrange(loss1, 'b t d -> b (t d)')
                loss2 = self.weight[:,1].view(1, 1, -1)
                loss2 = loss2.repeat(b, t, 1)
                loss2 = rearrange(loss2, 'b t d -> b (t d)')
            else:
                loss1, loss2 = self.weight[0], self.weight[1]
            ...
            ws = []
            if self.individual:
                y1, y2 = y1.view(b, t, d), y2.view(b, t, d)
                true = true.view(b, t, d)
                for i in range(d):                    
                    y1i, y2i, target = y1[:, :, i], y2[:, :, i], true[:, :, i]
                    ws.append(self.linear_regression(y1i, y2i, target))
                self.weight = torch.from_numpy(np.array(ws)).to(self.device)
            else:
                self.weight = torch.from_numpy(self.linear_regression(y1, y2, true)).to(self.device)
