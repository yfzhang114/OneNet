...
class net(nn.Module):
    ...
    def forward_weight(self, x, x_mark, g0, g1, g2):
        ...
        return y0 * g0 + y1 * g1 + y2 * g2, y0, y1, y2
...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.buffer0 = Buffer(int(1e5), self.device)       
        self.buffer1 = Buffer(int(1e5), self.device)       
        self.buffer2 = Buffer(int(1e5), self.device)       
        self.count = 0
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(vali_loader):
            ...
            weight = 1. / len(pred)
            pred = pred[0] * weight  + weight * pred[1] + weight * pred[2]
    ...
    def test(self, setting):
        ...
        if self.online == 'regressor':
            ...
            for p in self.model.parameters():
                p.requires_grad = False
        elif self.online == 'full':
            for p in self.model.patchtst.parameters():
                p.requires_grad = False
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, mode='train'):
        ...
        y0, y1, y2 = self.model.forward_individual(x, batch_x_mark)
        f_dim = -1 if self.args.features=='MS' else 0
        batch_y = batch_y[:,-self.args.pred_len:,f_dim:].to(self.device)
        return [y0, y1, y2], rearrange(batch_y, 'b t d -> b (t d)')
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            if not self.buffer1.is_empty():
                num_ori = 1
                loss0, loss1, loss2 = self.buffer0.get_data(num_ori), self.buffer1.get_data(num_ori), self.buffer2.get_data(num_ori)
                total_loss = loss1 + loss0 + loss2
                loss0, loss1, loss2 = 1 - loss0 / total_loss, 1 - loss1 / total_loss, 1 - loss2 / total_loss
                total_loss = loss1 + loss0 + loss2
                loss0, loss1, loss2 = loss0 / total_loss, loss1 / total_loss, loss2 / total_loss
            else:
                loss0 = 1./3; loss1 = 1./3; loss2 = 1./3
                
            if self.args.use_amp:
                with torch.cuda.amp.autocast():
                    # outputs = self.model(x, batch_x_mark)
                    outputs, y0, y1, y2 = self.model.forward_weight(x, batch_x_mark, loss0, loss1, loss2)
            else:
                outputs, y0, y1, y2 = self.model.forward_weight(x, batch_x_mark, loss0, loss1, loss2)

            loss0 = criterion(y0, true) 
            loss1 = criterion(y1, true) 
            loss2 = criterion(y2, true)
        ...
        self.buffer0.add_data(losses = loss0.detach())
        self.buffer1.add_data(losses = loss1.detach())
        self.buffer2.add_data(losses = loss2.detach())
        return outputs, rearrange(batch_y, 'b t d -> b (t d)')

    def _ol_one_batch_(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        true = rearrange(batch_y, 'b t d -> b (t d)').float().to(self.device)
        criterion = self._select_criterion()
